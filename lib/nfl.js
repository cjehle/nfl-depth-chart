...
function getAthlete(id) {
  if (athleteStore.size > 5000) athleteStore.clear(); // bound the cache (birthdays don't change; safe to drop)
  return cached(athleteStore, id, Infinity, async () => { const d = await fetchJson(athleteUrl(id)); return { dob: d.dateOfBirth || null }; });
}
...
function nflRecord(teamId) {
  return cached(nflRecordStore, teamId, 6 * 3600e3, async () => {
    try { const d = await fetchJson(`https://site.api.espn.com/apis/site/v2/sports/football/nfl/teams/${teamId}`); return d.team?.record?.items?.[0]?.summary || null; }
    catch { return null; }
  }).catch(() => null);
}
...
async function buildTeamData(teamId, year) {
  ...
  if (src.usedYear >= cur) env.record = await nflRecord(teamId);
  return env;
}
...
async function getTeamData(teamId, year, fresh) {
  ...
  try {
    return await cached(teamStore, key, DEPTH_TTL, async () => { const data = await buildTeamData(teamId, year); writeDisk(key, data); return data; });
  } catch (err) {
