...
const { fetchText, fetchJson, cached, writeDisk, readDisk, stats, injuryDetail, recordBuild } = require("./espn.js");
...
function buildRosterMap(roster, maddenMap) {
  ...
  for (const group of roster.athletes || []) {
    for (const a of group.items || []) {
      ...
      map.set(String(a.id), {
        // Fall back to type.description/name when ESPN omits .status, so a flagged player
        // always gets a badge (parity with the surface app's injuryStatus()).
        name, jersey: a.jersey || "", injury: injuries.length ? (injuries[0].status || injuries[0].type?.description || injuries[0].type?.name || null) : null,
...
function getNflverseSeason(year) {
  return cached(nflverseStore, year, Infinity, async () => {
    ...
    // ASSUMES no embedded newlines inside quoted fields (the whole-file parseCsv in
    // nfl-util.js does handle those). Verified benign in the nflverse depth-charts feed —
    // the only such cells are whitespace-only depth_position values that map to no slot.
...
function getNewFormatSeason(year) {
  return cached(newFormatStore, year, Infinity, async () => {
    ...
    // Fail LOUD if nflverse renames a load-bearing column: header.indexOf → -1 would otherwise
    // silently yield "—" rows for the whole season. Throwing here routes getTeamData to the
    // committed seed / last-good fallback (honest stale banner) instead of serving junk.
    const missing = ["team", "player_name", "pos_grp", "pos_abb"].filter((w) => idx[w] === -1);
    if (missing.length) throw new Error(`nflverse ${year} CSV is missing columns: ${missing.join(", ")} (format changed?)`);
    const rows = [];
    for (let i = headerEnd + 1; i < text.length;) {
      const nl = text.indexOf("\n", i); const end = nl === -1 ? text.length : nl;
...
async function getTeamData(teamId, year, fresh) {
  ...
  // A completed season is IMMUTABLE, so its depth chart is pre-baked into data/seed
  // (scripts/gen-nfl-history.js). Prefer that committed snapshot over a ~50MB runtime
  // nflverse CSV fetch+parse: it removes the biggest free-tier memory spike AND the
  // hardcoded github.com dependency from the request path. Unbaked seasons fall through
  // to the live nflverse build below, so nothing regresses.
  const past = year < currentNflSeason();
  ...
  try {
    // Past seasons cache forever (immutable); the current season keeps its DEPTH_TTL refresh.
    return await cached(teamStore, key, past ? Infinity : DEPTH_TTL, async () => {
      if (past) { const pre = readDisk(diskK); if (pre) return pre.data; } // committed snapshot → no 50MB fetch
      const data = await buildTeamData(teamId, year);
      // [durability] Silent-drift guard, mirroring the surface completeness gate (server.js).
      // If ESPN changes a field the builder reads but still returns HTTP 200, espnGroupEntries
      // drops the unreadable players and makeEnvelope yields an all-null envelope WITHOUT
      // throwing. Never persist/serve that as fresh — it would poison last-good and, because
      // the 200 counts as an upstream success, stay invisible to /healthz. recordBuild() feeds
      // the same rolling drift window the surface builder uses, so /healthz?strict=1 can now
      // catch NFL-only drift too. (No "thin" threshold for NFL — offense+defense+ST have no
      // single expected count across formations, so only a fully-empty build is flagged.)
      const empty = !data.offense && !data.defense && !data.specialTeams;
      recordBuild(empty, false);
      if (empty) {
        const hit = readDisk(diskK);
        if (hit) { console.error(`empty NFL build for ${diskK} (ESPN drift?) — serving ${hit.source}`); return { ...hit.data, stale: true, source: hit.source }; }
        throw new Error(`NFL build for ${diskK} returned no units (ESPN drift?)`);
      }
      writeDisk(diskK, data);
      return data;
    }, 400);
  } catch (err) {
    const hit = readDisk(diskK);
...
module.exports = {
  ...
  TEAM_BY_ID,
  buildTeamData, // used by scripts/gen-nfl-history.js to pre-bake completed seasons
};
