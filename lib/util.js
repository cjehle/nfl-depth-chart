...
// Parse a whole CSV string into an array of row objects. Strips a UTF-8 BOM,
// handles quoted fields (including embedded newlines), and — when `wanted` is
// given (an array/Set of column names) — keeps only those columns to save memory.
function parseCsv(text, wanted) {
  ...
  const keep = want ? header.map((h) => want.has(h)) : null;
  return rows.map((r) => {
    const obj = {};
    for (let i = 0; i < header.length; i++) {
      if (!keep || keep[i]) obj[header[i]] = r[i];
    }
    return obj;
  });
}
...
module.exports = {
  ...
  offenseKey, defenseCat, groupBy, splitIntoSpots,
};
