...
let _lastPrune = 0; // throttle the O(n) idle scan off the per-new-session hot path (see record())
  ...
  try {
    ...
    if (sid) {
      ...
      if (!s) {
        ...
        // Throttle: prune immediately if over the hard cap (safety), else at most every 5s —
        // don't run the whole-map idle scan on every single new session.
        if (state.sessions.size > SESSIONS_CAP || now - _lastPrune > 5000) { _lastPrune = now; pruneSessions(now); }
      } else {
        s.lastTs = now; s.pv++;
...
function serialize() {
  ...
  return JSON.stringify({ v: 1, days: state.days, sessions });
}
// Synchronous save — used ONLY on shutdown (SIGTERM/SIGINT), where a dying process must
// finish the write before exit.
function save() {
  try { fs.mkdirSync(DIR, { recursive: true }); fs.writeFileSync(FILE, serialize()); return true; } catch { return false; }
}
// Async save for the periodic (every-60s) flush, so serializing up to 90 days of histograms
// + up to SESSIONS_CAP live sessions never blocks the single event loop / stalls in-flight
// requests. An in-flight guard drops overlapping ticks (last-writer-wins is fine for a snapshot).
let _saving = false;
function saveAsync() {
  if (_saving) return;
  _saving = true;
  try {
    fs.mkdir(DIR, { recursive: true }, () => {
      let body; try { body = serialize(); } catch { _saving = false; return; }
      fs.writeFile(FILE, body, () => { _saving = false; });
    });
  } catch { _saving = false; }
}
function load() {
...
module.exports = { record, summary, save, saveAsync, load, _state: state, _hist: { newHist, histAdd, histPct } };
