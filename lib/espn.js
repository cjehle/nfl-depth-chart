...
// Counters surfaced on /healthz so free-tier hiccups aren't guesswork. The extra
// fields let /healthz report a real ok|degraded verdict (see server.js): fallback
// serves, empty builds, last-success/last-failure times, and a rolling per-minute
// window so a bad LAST HOUR isn't drowned by millions of historical successes.
const stats = {
  started: Date.now(), requests: 0, cacheHits: 0, upstreamOk: 0, upstreamFail: 0, rateLimited: 0,
  servedLastGood: 0, servedSeed: 0, emptyBuilds: 0, lastUpstreamOkAt: 0, lastUpstreamFailAt: 0,
};
// Rolling counters over the last RING_MIN minutes (fixed memory). Tracks HTTP ok/fail
// AND build outcomes (built vs empty), so /healthz can catch the silent-drift failure:
// ESPN returns 200 (fetch "ok") but the fields we read are gone, so builders produce
// EMPTY lineups — a spike the ok/fail counters alone can't see.
const RING_MIN = 15;
const ring = Array.from({ length: RING_MIN }, () => ({ minute: -1, ok: 0, fail: 0, built: 0, empty: 0, degraded: 0 }));
function slotNow() {
  const minute = Math.floor(Date.now() / 60000);
  const slot = ring[minute % RING_MIN];
  if (slot.minute !== minute) { slot.minute = minute; slot.ok = 0; slot.fail = 0; slot.built = 0; slot.empty = 0; slot.degraded = 0; }
  return slot;
}
function recordUpstream(ok) {
  const slot = slotNow();
  if (ok) { slot.ok++; stats.upstreamOk++; stats.lastUpstreamOkAt = Date.now(); }
  else { slot.fail++; stats.upstreamFail++; stats.lastUpstreamFailAt = Date.now(); }
}
// Record a completed lineup build: empty (0 chips) or degraded (well under its expected
// slot count). Degraded catches PARTIAL ESPN drift that empty-only counting would miss.
function recordBuild(isEmpty, isDegraded) {
  const slot = slotNow();
  slot.built++;
  if (isEmpty) { slot.empty++; stats.emptyBuilds++; }
  else if (isDegraded) { slot.degraded++; stats.degradedBuilds = (stats.degradedBuilds || 0) + 1; }
}
// Rolling error/empty/degraded rates + volume over the last RING_MIN minutes.
function recentUpstream() {
  const cutoff = Math.floor(Date.now() / 60000) - RING_MIN;
  let ok = 0, fail = 0, built = 0, empty = 0, degraded = 0;
  for (const s of ring) if (s.minute > cutoff) { ok += s.ok; fail += s.fail; built += s.built; empty += s.empty; degraded += s.degraded; }
  const total = ok + fail;
  return { ok, fail, total, errorRate: total ? fail / total : 0, built, empty, emptyRate: built ? empty / built : 0, degraded, degradedRate: built ? (empty + degraded) / built : 0 };
}
...
// ---------------------------------------------------------------------------
// Capped exponential backoff with FULL jitter between retries. Zero-delay retries
// (the old behavior) fire back-to-back — mostly tripling request rate without helping
// — and on an ESPN recovery dozens of concurrent builders (prewarm fans out ~30) would
// retry in lockstep: a self-synchronized thundering herd. Full jitter de-correlates them.
const retryBackoff = (attempt) => new Promise((r) => setTimeout(r, Math.random() * Math.min(1000, 150 * 2 ** attempt)));
// Global ceiling on concurrent outbound ESPN fetches across ALL builds + live traffic.
// Prewarm caps its BUILDERS at 6, but each build fans out 8-15 summary fetches and live
// requests stack on top, so real peaks can exceed ~90 sockets — the burst that trips ESPN
// rate-limiting and yields the thin/degraded builds the fallback then papers over. Every
// fetch holds one permit for its whole lifetime (incl. retries/backoff); excess calls queue.
const MAX_UPSTREAM = Number(process.env.MAX_UPSTREAM) || 24;
let inFlightUpstream = 0;
const upstreamWaiters = [];
function acquireUpstream() {
  if (inFlightUpstream < MAX_UPSTREAM) { inFlightUpstream++; return Promise.resolve(); }
  return new Promise((resolve) => upstreamWaiters.push(resolve));
}
function releaseUpstream() {
  const next = upstreamWaiters.shift();
  if (next) next(); else inFlightUpstream--; // hand the live permit straight to the next waiter
}
async function fetchText(url, opts) {
  await acquireUpstream();
  try { return await fetchTextCore(url, opts); } finally { releaseUpstream(); }
}
async function fetchTextCore(url, { ua = "sports-depth-chart-app", timeout = 15000, retries = 2, maxBytes = 40 * 1024 * 1024 } = {}) {
  let lastErr;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      const res = await fetch(url, { headers: { "User-Agent": ua, Accept: "application/json" }, signal: AbortSignal.timeout(timeout) });
      if (!res.ok) {
        if (res.status >= 500 && attempt < retries) { lastErr = new Error(`HTTP ${res.status}`); await retryBackoff(attempt); continue; }
      ...
      await retryBackoff(attempt); // de-correlated pause before retrying a transient network error
    }
  }
...
// ---------------------------------------------------------------------------
function cached(store, key, ttlMs, fn, cap = 0) {
  const hit = store.get(key);
  const hasValue = hit && "value" in hit;
  const fresh = hasValue && (ttlMs === Infinity || Date.now() - hit.time < ttlMs);
  if (fresh) return Promise.resolve(hit.value);

  // Start (or join) the single-flight rebuild for this key.
  const startRefresh = () => {
    const cur = store.get(key);
    if (cur && cur.promise) return cur.promise; // coalesce a burst into ONE fetch
    const promise = (async () => {
      const value = await fn();
      store.set(key, { time: Date.now(), value });
      evict(store, ttlMs, cap, key);
      return value;
    })();
    store.set(key, { ...(store.get(key) || {}), promise });
    promise.catch(() => {
      const c = store.get(key);
      if (!c || c.promise !== promise) return;
      if ("value" in c) delete c.promise; // a failed rebuild keeps the stale value
      else store.delete(key);             // nothing to fall back to → drop the placeholder
    });
    return promise;
  };

  // Stale-while-revalidate: when a usable (but expired) value exists, serve it NOW
  // and rebuild in the background — the upstream call is never on the visitor's
  // critical path. Only the genuinely cold path (no value at all) blocks on fn().
  if (hasValue) { startRefresh().catch(() => {}); return Promise.resolve(hit.value); }
  return startRefresh();
}
// Keep a Map-backed cache bounded: drop already-stale (not in-flight) entries
// first, then the oldest by insertion order — never the key just written, never an
// in-flight/refreshing entry. cap 0 = unbounded (for tiny, fixed-key-space stores).
function evict(store, ttlMs, cap, keep) {
  if (!cap || store.size <= cap) return;
  if (ttlMs !== Infinity) {
    const now = Date.now();
    for (const [k, e] of store) {
      if (store.size <= cap) break;
      if (k === keep || !e || !("value" in e) || e.promise) continue;
      if (now - e.time >= ttlMs) store.delete(k);
    }
  }
  for (const [k, e] of store) {
    if (store.size <= cap) break;
    if (k === keep || (e && e.promise)) continue;
    store.delete(k);
  }
}
...
// Canonical on-disk filename for a cache key. Trailing EMPTY segments are trimmed
// first, so a default lineup (no unit/year/formation) resolves to a STABLE name that
// the seed generator and the runtime agree on — e.g. both "nba:13:::" (runtime) and
// "nba:13:" (older generator) map to nba_13.json. Without the trim they diverged
// (nba_13___.json vs nba_13_.json) and the committed seed was never found.
const safeKey = (key) => key.replace(/:+$/, "").replace(/[^\w.-]/g, "_") + ".json";
// The ONE place a surface lineup's cache/disk key is built — shared by the server and
// scripts/gen-seeds so their filenames can never drift apart again.
const lineupKey = (sport, teamId, unit, year, formation) => `${sport}:${teamId}:${unit || ""}:${year || ""}:${formation || ""}`;
...
// Returns { data, source } or null. source: "last-good" = a fresh copy THIS instance
// wrote before the upstream broke; "seed" = the committed baseline (the only durable
// copy after a Render cold start wipes /tmp). Lets callers tag responses as stale.
function readDisk(key) {
  const name = safeKey(key);
  try { const data = JSON.parse(fs.readFileSync(path.join(DISK_DIR, name), "utf8")); stats.servedLastGood++; return { data, source: "last-good" }; } catch {}
  try { const data = JSON.parse(fs.readFileSync(path.join(SEED_DIR, name), "utf8")); stats.servedSeed++; return { data, source: "seed" }; } catch { return null; }
}
...
// Immutable-summary cache: completed-game box scores never change, so cache the raw summary
// JSON by event. Infinity TTL (never stale) + single-flight (concurrent builds needing the
// same event share one fetch) + bounded cap. Best-effort opts match the aggregation sites.
const summaryStore = new Map();
function fetchSummaryCached(sport, league, eventId) {
  return cached(summaryStore, `${sport}:${league}:${eventId}`, Infinity, () => fetchJson(summaryUrl(sport, league, eventId), { timeout: 8000, retries: 0 }), 300);
}
// ESPN headshot + web player-page slugs. Soccer uses the sport-level "soccer";
// everyone else uses the league slug.
const webSlug = (cfg) => (cfg.espn.sport === "soccer" ? "soccer" : cfg.espn.league);
function playerLinks(cfg, id) {
  if (!id) return { photo: null, espnUrl: null };
  const s = webSlug(cfg);
  return {
    photo: `https://a.espncdn.com/i/headshots/${s}/players/full/${id}.png`,
    espnUrl: `https://www.espn.com/${s}/player/_/id/${id}`,
  };
}
...
// Richer injury info for the popover: what it is + expected return, when ESPN has it.
function injuryDetail(a) {
  const inj = (a.injuries || [])[0];
  if (!inj) return null;
  const detail = inj.details?.type || inj.type?.description || inj.shortComment || inj.longComment || "";
  const ret = inj.details?.returnDate || "";
  return detail || ret ? { detail: String(detail).slice(0, 120), ret } : null;
}
...
// Season-ending / unavailable statuses that shouldn't be shown as the CURRENT starter
// (day-to-day / questionable / doubtful do NOT churn the starter — those are game-time).
// Used by the single-face-per-position builders to promote the next available player.
const OUT_STATUS = /out|injured reserve|\bir\b|suspen|\bpup\b|\bnfi\b/i;
function healthyFaceIdx(players, baseIdx) {
  if (!players[baseIdx] || !OUT_STATUS.test(String(players[baseIdx].injury || ""))) return baseIdx;
  for (let i = baseIdx + 1; i < players.length; i++) if (!OUT_STATUS.test(String(players[i].injury || ""))) return i;
  return baseIdx; // everyone at this spot is unavailable → keep the ranked starter (still flagged)
}
// ---------------------------------------------------------------------------
...
async function resolveDepthLineup(cfg, team, season) {
  ...
  // The roster doesn't depend on which season's depth chart we find, so fetch it in
  // parallel with the probe loop instead of serially after it. (.catch keeps it
  // abandonable if the probe throws first — no unhandled rejection.)
  const rosterP = fetchJson(rosterUrl(sport, league, team.id)).catch(() => null);
  ...
  const rosterJson = await rosterP;
  if (!rosterJson) throw new Error(`No roster available for ${team.name}.`);
  ...
  for (const spec of cfg.layout) {
    ...
    const faceIdx = healthyFaceIdx(players, Math.min((spec.faceRank || 1) - 1, players.length - 1)); // skip a season-ending-injured/suspended starter
    chips.push({
...
function soccerSlot(posAbbr) {
  ...
  else if (["FB", "WB"].includes(base)) { band = 1; wide = true; if (side === 0) side = 1; } // fullbacks/wingbacks are the WIDEST defenders: keep an explicit -L/-R side; lean a bare code off-center so it still sorts outside the center-backs
  else if (["CB", "CD", "SW", "D"].includes(base)) { band = 1; center = true; }
...
// Choose which completed games to sample for a "typical lineup". `completed` is
// newest-first. Current season → the newest n (recency is what "typical now" means).
// A PAST season → a window centered on MID-season: the final games of a finished
// season are rest days / tanking / playoff rotation, which misrepresent the norm.
function pickGames(completed, n, historical) {
  if (!historical || completed.length <= n) return completed.slice(0, n);
  const asc = [...completed].reverse(); // oldest-first
  const start = Math.max(0, Math.floor(asc.length / 2) - Math.floor(n / 2));
  return asc.slice(start, start + n).reverse(); // centered window, back to newest-first
}
...
function placeStarters(entries) {
  ...
  // A back line with an ODD count seats a defender dead-center (x≈50), directly
  // under the GK (also x≈50), so the two chips collide vertically. An EVEN line —
  // the common 4-at-the-back — straddles x≈50 with the GK tucked between the CBs,
  // so it is left exactly as-is. Only an odd back line gets extra clearance.
  const backBand = outfield[0];
  const centralBack = outfield.length > 0 && entries.filter((e) => e.band === backBand).length % 2 === 1;
  const yFor = (band) => {
    if (band === 0) return 8; // GK — kept clear of the team name band
    const k = outfield.indexOf(band), B = outfield.length;
    const base = B <= 1 ? 30 : 16 + (50 - 16) * (k / (B - 1)); // back line ≈16 → front line ≈50
    // Drop only an odd back line from y16 → y20 (a full chip-height below the GK),
    // and only in 3-/5-back shapes (≤3 lines), where the next line sits at y≈33 and
    // stays >1 chip clear. Deeper shapes (a screening DM at ≈y27) are left untouched.
    return band === backBand && centralBack && B >= 2 && B <= 3 ? 20 : base;
  };
  const byBand = {};
...
async function resolveMatchLineup(cfg, team, season, formationOverride) {
  const { sport, league } = cfg.espn;
  const historical = !!season;
  // Schedule is independent of the roster, so start it NOW — the two ESPN round trips
  // overlap instead of running serially (one full RTT off every cold build). (.catch so
  // an abandoned promise can't reject unhandled if the roster path throws first.)
  const schedP = fetchJson(scheduleUrl(sport, league, team.id, season)).catch(() => null);
  ...
  try {
    const sched = await schedP;
    // Completed games, newest-first, TBD/null-date events dropped (NaN-safe sort + "as of").
    const completed = ((sched && sched.events) || []).filter((e) => e.competitions?.[0]?.status?.type?.completed && Number.isFinite(Date.parse(e.date)))
      .sort((a, b) => new Date(b.date) - new Date(a.date));
    const done = pickGames(completed, 8, historical);
    if (done.length) {
      when = done[0].date || null;
      // Pull each match's box in parallel; any that fail are simply skipped.
      // Best-effort per-game boxscores: tighter than the 15s/2-retry default so one
      // slow/flaky game can't stall the whole aggregation (it runs under Promise.all,
      // so the slowest gates the build) — a skipped game is absorbed by the multi-game
      // aggregation + fallback. Caps the cold-build tail and kills 3x retry amplification
      // during an ESPN 5xx wobble across the ~30-build prewarm fan-out.
      // ...and via the immutable-summary cache: a completed box score never changes, so a
      // team's own periodic refresh reuses 7-of-8 unchanged games and the two teams that
      // played each other share ONE fetch (single-flight) instead of double-pulling.
      const sums = await Promise.all(done.map((e) => fetchSummaryCached(sport, league, e.id).catch(() => null)));
      ...
      if (nMatches) {
        // Most common recent formation → line counts (or an explicit override the
        // user picked from the Formation dropdown). First digit = defenders, last =
        // forwards, everything between collapses into midfield.
        formation = formationOverride || Object.entries(formCount).sort((a, b) => b[1] - a[1])[0]?.[0] || "4-3-3";
        ...
        if (formationOverride) {
          // OVERRIDE re-shapes the XI instead of only relabeling it: pack the GK +
          // 10 outfielders into EXACTLY defN/midN/fwdN lines and set each player's
          // band to the LINE they land in (not their natural group). Pass 1 fills
          // each line from its own group's best scorers; pass 2 fills any short line
          // from the best leftover outfielders, sending each to the open line nearest
          // its natural band. Natural pos preferred, then score; L/R hint preserved.
          const lines = [{ band: 1, grp: "Defense", n: defN }, { band: 3, grp: "Midfield", n: midN }, { band: 5, grp: "Forward", n: fwdN }];
          const filled = { 1: 0, 3: 0, 5: 0 };
          const put = (p, band) => { p.band = band; chosen.push(p); have.add(p.id); if (band) filled[band]++; };
          if (byGrp.Goalkeeper[0]) put(byGrp.Goalkeeper[0], 0);
          for (const t of lines) for (const p of byGrp[t.grp]) { if (filled[t.band] >= t.n) break; if (!have.has(p.id)) put(p, t.band); }
          for (const p of players.filter((q) => !have.has(q.id) && q.grp !== "Goalkeeper").sort((a, b) => b.score - a.score)) {
            const open = lines.filter((t) => filled[t.band] < t.n);
            if (!open.length) break;
            const nat = bandForGrp[p.grp] ?? 3;
            open.sort((a, b) => Math.abs(a.band - nat) - Math.abs(b.band - nat) || a.band - b.band);
            put(p, open[0].band);
          }
        } else {
        ...
        // Line each player onto the band we slotted them into (match-to-match position
        // labels are noisy): an override uses the packed band above, the default the
        // natural group. L/R hint and natural label/grp are kept either way.
        const entries = chosen.map((p) => {
          ...
          return { bio, posAbbr: p.modePos, band: p.band ?? bandForGrp[p.grp] ?? 3, side: p.side, xhint: p.xhint, fp: p.fp, grp: p.grp };
        });
        placeStarters(entries);
        entries.sort((a, b) => a.band - b.band || a.x - b.x);
        // The depth behind each starter: today's squad by line (byGroup) for the current
        // season, but for a PAST season the season's OWN aggregated squad (byGrp) — never
        // today's roster, which would list present-day players as that season's depth.
        const depthByGrp = historical
          ? Object.fromEntries(Object.entries(byGrp).map(([g, arr]) => [g, arr.map((p) => ({ id: p.id, name: p.name || "—", jersey: "", injury: null, pos: p.modePos || "", extra: "" }))]))
          : byGroup;
        chips = entries.map((e, i) => {
          const rest = (depthByGrp[e.grp] || []).filter((p) => p.id !== e.bio.id);
  ...
  // When this projected-from-CURRENT-roster fallback is reached for a PAST season (ESPN had
  // no completed-match lineup for that year), say so — don't stamp today's squad as that season.
  return { formation: "4-3-3", subtitle: historical ? `Current roster · no ${season} data` : "Projected XI · 4-3-3", chips };
}
...
async function statScores(sport, league, season, ids) {
  ...
  async function worker() {
    while (i < ids.length) {
      const id = ids[i++];
      try {
        map.set(id, await cached(statStore, `${league}:${season}:${id}`, 24 * 3600e3, async () => {
        ...
        }, 2000)); // 24h TTL (a mid-season stat correction eventually lands) + bounded
...
async function resolveStatRankedLineup(cfg, team, unit) {
  ...
  for (const spec of cfg.layout) {
    ...
    const faceIdx = healthyFaceIdx(players, Math.min(base, players.length - 1)); // skip a season-ending-injured/suspended starter
...
function collegePlayer(a, cfg) {
  ...
  return {
    ...
    injury: injuryStatus(a), injuryDetail: injuryDetail(a),
...
async function resolveRosterLineup(cfg, team, unit, formation) {
  const { sport, league } = cfg.espn;
  ...
  // Per-unit package (CFB): the chosen offense personnel / defense front picks the
  // slot set + coordinates; otherwise the unit's default layout.
  const pkg = formation && cfg.packages && cfg.packages[unit] && cfg.packages[unit][formation];
  const layout = pkg || (cfg.dualUnit ? cfg.layouts[unit === "defense" ? "defense" : "offense"] : cfg.layout);
  const chips = [];
  if (cfg.surface === "court") {
    // Basketball roster fallback: build a DISTINCT five, backfilling a slot whose
    // position bucket is empty from surplus players (extra guards fill a wing, etc.)
    // so every team yields 5 chips, and give each slot its own depth list.
    const used = new Set();
    const rankB = { center: 0, forward: 1, guard: 2 };
    const takeFrom = (bkt) => { const p = (buckets[bkt] || []).find((x) => !used.has(x.id)); if (p) used.add(p.id); return p || null; };
    const takeAny = () => { for (const k of Object.keys(buckets)) { const p = buckets[k].find((x) => !used.has(x.id)); if (p) { used.add(p.id); return p; } } return null; };
    const faceFor = {};
    for (const spec of [...layout].sort((a, b) => (rankB[a.bucket] ?? 1) - (rankB[b.bucket] ?? 1))) faceFor[spec.key] = takeFrom(spec.bucket) || takeAny();
    const faceIds = new Set(Object.values(faceFor).filter(Boolean).map((f) => f.id));
    const BLABEL = { guard: "Guard", forward: "Forward", center: "Center" }, BGROUP = { guard: "Guards", forward: "Forwards", center: "Center" };
    for (const spec of layout) {
      const face = faceFor[spec.key]; if (!face) continue;
      const bkt = cfg.bucket(face.pos) || spec.bucket;
      // depth = this player's position bucket, excluding OTHER slots' faces (so PG/SG aren't identical).
      const rest = (buckets[bkt] || []).filter((p) => p.id === face.id || !faceIds.has(p.id));
      const players = rest.map((p, i) => ({ rank: i + 1, ...p }));
      const matched = bkt === spec.bucket;
      chips.push({ key: spec.key, label: matched ? spec.label : (BLABEL[bkt] || spec.label), group: matched ? spec.group : (BGROUP[bkt] || spec.group), x: spec.x, y: spec.y, face: { rank: 1, ...face }, players });
    }
  } else {
    for (const spec of layout) {
      const list = (buckets[spec.bucket] || []).map((p, i) => ({ rank: i + 1, ...p }));
      // Leave a spot EMPTY when the bucket has no Nth player, rather than clamping to the last
      // one — clamping duplicated the same player onto every surplus spot (two chips, same guy,
      // stacked). A thin position group now shows fewer chips instead of dupes.
      if ((spec.faceRank || 1) > list.length) continue;
      const faceIdx = (spec.faceRank || 1) - 1;
      chips.push({ key: spec.key, label: spec.label, group: spec.group, x: spec.x, y: spec.y, face: list[faceIdx], players: list });
    }
  }
  const pkgWord = pkg && cfg.unitFormationLabels ? `${cfg.unitFormationLabels[unit] || "Formation"} ${formation} · ` : "";
  const unitWord = cfg.dualUnit ? (unit === "defense" ? "Defense" : "Offense") + " · " : "";
  return { formation: pkg ? formation : null, subtitle: `${unitWord}${pkgWord}${cfg.rosterLabel || "roster by class"}`, chips };
}
...
async function resolveBoxStartLineup(cfg, team, season) {
  const { sport, league } = cfg.espn;
  const historical = !!season;
  // Overlap the schedule fetch with the roster fetch (independent) — one RTT off cold builds.
  const schedP = fetchJson(scheduleUrl(sport, league, team.id, season)).catch(() => null);
  const rosterJson = await fetchJson(rosterUrl(sport, league, team.id));
  const rosterMap = buildRosterMap(rosterJson, cfg);
  ...
  const fallback = async () => {
    if (cfg.boxFallback === "depth") {
      const d = await resolveDepthLineup(cfg, team, season);
      if (!season) d.subtitle = "projected starting five · ESPN depth chart"; // offseason: no box scores yet
      return d;
    }
    const r = await resolveRosterLineup(cfg, team);
    // Historical fallback uses TODAY's roster (past box scores too sparse) — label it honestly
    // rather than pass the current squad off as that past season.
    if (historical) r.subtitle = `Current roster · no ${season} data`;
    return r;
  };
  ...
  try {
    ...
    const completed = ((sched && sched.events) || []).filter((e) => e.competitions?.[0]?.status?.type?.completed && Number.isFinite(Date.parse(e.date)))
      .sort((a, b) => new Date(b.date) - new Date(a.date));
    const done = pickGames(completed, historical ? 15 : 10, historical);
    if (done.length) {
      when = done[0].date || null;
      // Best-effort per-game boxscores: tighter than the 15s/2-retry default so one
      // slow/flaky game can't stall the whole aggregation (it runs under Promise.all,
  ...
  // STABLE partition: place true centers first (keeping their existing score
  // order), then everyone else in score order. A pure score-order pass lets
  // three higher-scoring forwards greedily fill PF/SF/C and shove a lower-usage
  // true center out to a guard wing while a forward stands in the paint.
  // Fronting only the centers fixes that WITHOUT reordering guards/forwards
  // among themselves — filter() preserves relative order — so a five with no
  // true C, and any five whose C already claims the paint, is placed identically.
  const placeOrder = [...five.filter((p) => p.bucket === "center"), ...five.filter((p) => p.bucket !== "center")];
...
// the same payload as the record) → { opp, oppId, homeAway: "vs"|"@", date } or null.
function parseNext(team, myId) {
  ...
  return {
    ...
    // Opponent team id — lets a `defaultVsNext` sport open on this team's next opponent
    // (surface/app.js). Null-safe: absent for a TBD/bye opponent → the client keeps its default.
    oppId: (opp.team && opp.team.id != null) ? String(opp.team.id) : null,
...
function teamRecord(sport, league, teamId) {
  ...
  }, 2000).catch(() => ({ record: null, next: null }));
}
...
// Expected number of on-surface slots for a build — used to detect a THIN/degraded
// build (partial ESPN drift). match=11; per-unit roster (CFB)=its unit layout; else cfg.layout.
function expectedSlotCount(cfg, unit) {
  if (cfg.kind === "match") return 11;
  if (Array.isArray(cfg.layout)) return cfg.layout.length;
  if (cfg.layouts) { const u = unit && cfg.layouts[unit] ? unit : Object.keys(cfg.layouts)[0]; return (cfg.layouts[u] || []).length || 11; }
  return 11;
}
// ---------------------------------------------------------------------------
// PUBLIC: build one team's lineup payload (cached per team, daily).
// ---------------------------------------------------------------------------
async function buildLineup(cfg, team, unit, season, formation) {
  ...
  // A formation override must be one the config declares: a whole-team option
  // (soccer) or a per-unit package (CFB offense personnel / defense front).
  const form = formation && ((cfg.formations || []).includes(formation) || (cfg.packages && cfg.packages[unit] && cfg.packages[unit][formation])) ? formation : null;
  const [src, ctx, ranks] = await Promise.all([
    cfg.kind === "match" ? resolveMatchLineup(cfg, team, hist, form)
      : cfg.kind === "statrank" ? resolveStatRankedLineup(cfg, team, unit)
        : cfg.kind === "boxstart" ? resolveBoxStartLineup(cfg, team, hist)
          : cfg.kind === "roster" ? resolveRosterLineup(cfg, team, unit, form)
  ...
  // Record the build outcome into the rolling window so /healthz can see silent ESPN
  // drift: EMPTY (0 chips) or DEGRADED (well below the expected slot count — partial
  // drift that empty-only counting would miss). 200-OK fetches don't reveal either.
  const nChips = (src && src.chips && src.chips.length) || 0;
  const expectedSlots = expectedSlotCount(cfg, unit);
  recordBuild(nChips === 0, nChips > 0 && nChips < 0.6 * expectedSlots);
  ...
  // Headshot + ESPN player-page URLs are fully derivable from the athlete id + the
  // sport's web slug, so we no longer stamp them onto every player (they were ~40% of
  // the raw payload). The client rebuilds them from `id` + CONFIG.webSlug — see
  // photoFor()/espnFor() in surface/app.js. img-src already allows *.espncdn.com.
  return {
    sport: cfg.key,
    surface: cfg.surface,
    ...
    chips: src.chips,
    expectedSlots, // how many chips a healthy build should have (thin-build gate in getLineup)
...
module.exports = {
  ...
  lineupKey, safeKey, recentUpstream, recordBuild, // shared key builder + health-verdict inputs (recordBuild lets the NFL engine feed the same drift ring)
  webSlug, injuryDetail, // web-slug (for the config island) + richer injury info (NFL popover parity)
  // exported for possible reuse/testing
  buildRosterMap, rankedPlayers, soccerSlot, soccerGroup, placeStarters,
  healthyFaceIdx, expectedSlotCount, // injury-aware starter + thin-build gate
};
