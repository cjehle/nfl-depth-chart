...
async function resolveDepthLineup(cfg, team) {
  ...
  let depth = null;
  // Prefer the current season (correct for in-season sports like MLB), then the
  // upcoming one, then last — first with a populated chart wins.
  for (const year of [nowY, nowY + 1, nowY - 1]) {
    try {
...
function teamRecord(sport, league, teamId) {
  return cached(recordStore, `${league}:${teamId}`, CONTEXT_TTL, async () => {
    try { const d = await fetchJson(`${site}/${sport}/${league}/teams/${teamId}`); return d.team?.record?.items?.[0]?.summary || null; }
    catch { return null; }
  }).catch(() => null);
}
...
function rankingsMap(sport, league) {
  return cached(rankStore, league, CONTEXT_TTL, async () => {
    try {
      const d = await fetchJson(`${site}/${sport}/${league}/rankings`);
