...
async function buildLineup(cfg, team, unit, season, formation) {
  ...
  // Attach an ESPN headshot URL + player-page link (client falls back gracefully
  // if a headshot 404s). Enables the richer depth-chart popover.
  for (const c of src.chips || []) {
    const add = (p) => { if (p && p.id) { const l = playerLinks(cfg, p.id); p.photo = l.photo; p.espnUrl = l.espnUrl; } };
    add(c.face);
    for (const p of c.players || []) add(p);
  }
  return {
    sport: cfg.key,
    surface: cfg.surface,
