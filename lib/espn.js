...
const RING_MIN = 15;
const ring = Array.from({ length: RING_MIN }, () => ({ minute: -1, ok: 0, fail: 0, built: 0, empty: 0 }));
function slotNow() {
  const minute = Math.floor(Date.now() / 60000);
  const slot = ring[minute % RING_MIN];
  if (slot.minute !== minute) { slot.minute = minute; slot.ok = 0; slot.fail = 0; slot.built = 0; slot.empty = 0; }
  return slot;
}
...
// Record a completed lineup build and whether it came back empty (no chips).
function recordBuild(isEmpty) {
  const slot = slotNow();
  slot.built++;
  if (isEmpty) { slot.empty++; stats.emptyBuilds++; }
}
// Rolling error/empty rate + volume over the last RING_MIN minutes (ignores stale slots).
function recentUpstream() {
  const cutoff = Math.floor(Date.now() / 60000) - RING_MIN;
  let ok = 0, fail = 0, built = 0, empty = 0;
  for (const s of ring) if (s.minute > cutoff) { ok += s.ok; fail += s.fail; built += s.built; empty += s.empty; }
  const total = ok + fail;
  return { ok, fail, total, errorRate: total ? fail / total : 0, built, empty, emptyRate: built ? empty / built : 0 };
}
...
async function buildLineup(cfg, team, unit, season, formation) {
  ...
  // An empty build = upstream answered but produced no lineup — the signature of a
  // silent ESPN schema/shape drift (200 OK, but the fields we read are gone). Recorded
  // into the rolling window so /healthz degrades on this even though no fetch "failed".
  recordBuild(!src || !src.chips || !src.chips.length);
