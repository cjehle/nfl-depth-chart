...
async function resolveDepthLineup(cfg, team) {
  ...
  for (const year of [cur, cur - 1]) {
    try {
      ...
      if (d && (d.items || []).length && d.items[0].positions && Object.keys(d.items[0].positions).length) { depth = d; break; }
    } catch {}
  }
...
async function resolveMatchLineup(cfg, team) {
  ...
  try {
    ...
    if (done.length) {
      ...
      if (nMatches) {
        ...
        // score them by starts (recency breaks ties — smaller lastIdx wins).
        const players = [...agg.values()].map((r) => {
  ...
  if (chips.length) {
    return { formation, subtitle: `${formation} · typical XI (last ${nMatches} matches)`, chips, updatedMatch: when };
  }
...
// ---------------------------------------------------------------------------
async function resolveBoxStartLineup(cfg, team) {
  const { sport, league } = cfg.espn;
  const rosterJson = await fetchJson(rosterUrl(sport, league, team.id));
  const rosterMap = buildRosterMap(rosterJson, cfg);
  // Display bios (class year for college), and the roster grouped into the
  // court's buckets for the depth popover behind each spot.
  ...
  const fallback = () => (cfg.boxFallback === "depth" ? resolveDepthLineup(cfg, team) : resolveRosterLineup(cfg, team));
  ...
  if (agg.size < 4) return fallback(); // not enough returning starters to trust
  ...
  return { formation: null, subtitle: `typical starting five · last ${nGames} games`, chips, updatedMatch: when };
}
...
function teamRecord(sport, league, teamId) {
  return cached(recordStore, `${league}:${teamId}`, CONTEXT_TTL, async () => {
    try { const d = await fetchJson(`${site}/${sport}/${league}/teams/${teamId}`, { timeout: 5000, retries: 0 }); return d.team?.record?.items?.[0]?.summary || null; }
    catch { return null; }
  }).catch(() => null);
}
...
// ---------------------------------------------------------------------------
async function buildLineup(cfg, team, unit) {
  const isCollege = cfg.key === "cfb" || cfg.key === "cbb";
  const [src, record, ranks] = await Promise.all([
    cfg.kind === "match" ? resolveMatchLineup(cfg, team)
      : cfg.kind === "statrank" ? resolveStatRankedLineup(cfg, team, unit)
        : cfg.kind === "boxstart" ? resolveBoxStartLineup(cfg, team)
          : cfg.kind === "roster" ? resolveRosterLineup(cfg, team, unit)
            : resolveDepthLineup(cfg, team),
  ...
  // Attach video-game overall ratings (MLS → EA FC, MLB → The Show) from the
  // committed maps — zero runtime fetching. NFL/Madden is handled in lib/nfl.js.
  const ratingLabel = ratings.publisher(cfg.key);
  if (ratingLabel) {
    for (const c of src.chips || []) {
      if (c.face) c.face.overall = ratings.ratingFor(cfg.key, c.face.name);
      for (const p of c.players || []) p.overall = ratings.ratingFor(cfg.key, p.name);
    }
  }
  return {
    sport: cfg.key,
    ...
    team: { id: team.id, name: team.name, abbr: team.abbr, color: team.color, alt: team.alt, logo: team.logo, record: record || null, rank: ranks.get(String(team.id)) || null },
