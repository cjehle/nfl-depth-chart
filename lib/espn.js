...
function collegePlayer(a, cfg) {
  ...
  return {
    ...
    injury: injuryStatus(a), age: null,
    classYear: a.experience?.abbreviation || CLASS_ABBR[years] || "",
    classOrder: years,
...
async function resolveRosterLineup(cfg, team, unit) {
  ...
  return { formation: null, subtitle: `${unitWord}roster by class`, chips };
}
...
async function buildLineup(cfg, team, unit) {
  ...
  const [src, record, ranks] = await Promise.all([
    cfg.kind === "match" ? resolveMatchLineup(cfg, team)
      : cfg.kind === "statrank" ? resolveStatRankedLineup(cfg, team)
