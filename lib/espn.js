...
async function resolveMatchLineup(cfg, team) {
  ...
  if (chips.length) {
    return { formation, subtitle: `${formation} · typical XI (last ${nMatches} match${nMatches === 1 ? "" : "es"})`, chips, updatedMatch: when };
  }
...
async function resolveBoxStartLineup(cfg, team) {
  ...
  return { formation: null, subtitle: `typical starting five · last ${nGames} game${nGames === 1 ? "" : "s"}`, chips, updatedMatch: when };
}
...
async function buildLineup(cfg, team, unit) {
  ...
  return {
    ...
    team: { id: team.id, name: team.name, abbr: team.abbr, color: team.color, alt: team.alt, logo: team.logo, record: record || null, rank: ranks.get(String(team.id)) || null, next: next || null },
