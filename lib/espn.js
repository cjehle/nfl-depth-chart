...
// Rolling upstream ok/fail counts over the last RING_MIN minutes (fixed memory).
const RING_MIN = 15;
const ring = Array.from({ length: RING_MIN }, () => ({ minute: -1, ok: 0, fail: 0 }));
function recordUpstream(ok) {
  const minute = Math.floor(Date.now() / 60000);
  const slot = ring[minute % RING_MIN];
  if (slot.minute !== minute) { slot.minute = minute; slot.ok = 0; slot.fail = 0; }
...
// Upstream error rate + volume over the last RING_MIN minutes (ignores stale slots).
function recentUpstream() {
  const cutoff = Math.floor(Date.now() / 60000) - RING_MIN;
  let ok = 0, fail = 0;
  for (const s of ring) if (s.minute > cutoff) { ok += s.ok; fail += s.fail; }
  const total = ok + fail;
  return { ok, fail, total, errorRate: total ? fail / total : 0 };
}
...
async function buildLineup(cfg, team, unit, season, formation) {
  ...
  // silent ESPN schema/shape drift (200 OK, but the fields we read are gone). Count
  // it so /healthz can flag this even though no fetch "failed".
  if (!src || !src.chips || !src.chips.length) stats.emptyBuilds++;
